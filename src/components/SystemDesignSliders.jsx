...
import './SystemDesignSliders.css'
...
function SystemDesignSliders() {
  const [activeDim, setActiveDim] = useState('consistency')
  const [sliderValue, setSliderValue] = useState(1)
  const [modalOpen, setModalOpen] = useState(false)
  const [modalData, setModalData] = useState({ type: '', item: null })

  const dim = data[activeDim]
  const stop = dim.stops[sliderValue]
  ...
  return (
    <div className="sds-widget">
      <div className="sds-header">
        <h2 className="sds-title">System Design Trade-off Matrix</h2>
        <p className="sds-subtitle">Adjust constraints to see architectural shifts.</p>
      </div>
      ...
      <div className="sds-tabs">
        {Object.keys(data).map(key => (
          <button
            key={key}
            className={`sds-tab-btn ${key === activeDim ? 'active' : ''}`}
      ...
      <div className="sds-display">
        <div className="sds-dim-title">{dim.title}</div>
        <p className="sds-dim-desc">{dim.desc}</p>
        ...
        <div className="sds-slider-container">
          <input
            ...
            className="sds-range-input"
          />
          <div className="sds-range-labels">
            {dim.stops.map((s, i) => (
              <div
                key={i}
                className={`sds-range-label ${i === sliderValue ? 'active' : ''}`}
      ...
      <div className="sds-content">
        <div className="sds-section-header">
          <div>
            <span className="sds-pattern-label">Architecture Pattern</span>
            <h3 className="sds-pattern-name">{stop.pattern}</h3>
          </div>
        </div>

        <div className="sds-tags">
          {stop.tech.map((item, i) => (
            <button
              key={`tech-${i}`}
              className="sds-tag sds-tag-tech"
              onClick={() => openModal('tech', item)}
            >
              {item.name}
            </button>
          ))}
          {(stop.concepts || []).map((item, i) => (
            <button
              key={`concept-${i}`}
              className="sds-tag sds-tag-concept"
              onClick={() => openModal('concept', item)}
            >
              {item.term}
            </button>
          ))}
        ...
        <div className="sds-example">
          <span className="sds-example-label">Real-World Example</span>
          <h4 className="sds-example-app">{stop.exampleApp}</h4>
          <p className="sds-example-desc">{stop.exampleDesc}</p>
        </div>

        <div className="sds-grid">
          <div className="sds-card sds-card-pros">
            <h4 className="sds-card-title">✓ Gains</h4>
            <ul className="sds-list">
              {stop.pros.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
          <div className="sds-card sds-card-cons">
            <h4 className="sds-card-title">✕ Sacrifices</h4>
            <ul className="sds-list">
              {stop.cons.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
        ...
        <div className="sds-staff-note">
          <span className="sds-staff-label">⚠ Staff Engineer Insight</span>
          <span>{stop.note}</span>
        </div>
      </div>
      ...
      {/* Modal */}
      <div
        className={`sds-modal-overlay ${modalOpen ? 'open' : ''}`}
        onClick={(e) => e.target === e.currentTarget && closeModal()}
      >
        <div className="sds-modal">
          <div className={`sds-modal-header ${modalData.type}`}>
            <button className="sds-modal-close" onClick={closeModal}>×</button>
            <span className="sds-modal-type">
              {modalData.type === 'tech' ? 'Technology' : 'Concept'}
            </span>
            <h3 className="sds-modal-title">
              {modalData.item && (modalData.type === 'tech' ? modalData.item.name : modalData.item.term)}
            </h3>
          </div>
          <div className="sds-modal-body">
            <div className="sds-modal-def">
              {modalData.item && (modalData.type === 'tech' ? modalData.item.desc : modalData.item.def)}
            </div>
            {modalData.type === 'tech' && modalData.item?.why && (
              <div className="sds-modal-why-box">
                <span className="sds-modal-why-label">Why it fits here</span>
                <p className="sds-modal-why-text">{modalData.item.why}</p>
              </div>
            )}
          </div>
        </div>
...
export default SystemDesignSliders
